...
public interface UserMapper {
    ...
    List<PageData> selectListBus(Map<String, String> whereMap);
    ...
    //注册去重======通过类型和电话和楼盘
    @Select("SELECT id,uuid,nickname,type,tel,password,ifnull(birthday,'') as birthday,ifnull(email,'') as email,ifnull(sex,'') as sex,ifnull(header_url,'') as headerUrl from t_user WHERE type=#{type} and tel = #{tel}")
    ...
    @Select("SELECT * " +
            " from t_user  " +
            " WHERE tel=#{tel} and type = #{type}")
    PageData selectByPhoneAndType(@Param("tel") String tel, @Param("type") String type);

    @Update("update t_user set nickname=#{nickname},tel=#{tel} where id = #{id}")
    int updateBus(@Param("id") String id,  @Param("nickname") String nickname, @Param("tel") String tel);
